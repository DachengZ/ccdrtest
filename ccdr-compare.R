...
compare.edgeList <- function(estL, trueL, o = NULL) {
    ...
    nn <- length(trueL)
    if(length(estL) != nn) stop(paste0("Two edgeLists have different number of nodes.", 
                                       "True graph has ", nn, "; estimated graph has", length(estL), "."))
    if(!is.null(o)) {
...
Qcompare.edgeList <- function(estL, q = NULL, trueL) {
    ## edgeList from sparsebnUtils
    ## i %in% edL[[j]] means there is an edge i -> j
    nn <- length(trueL)
    if(length(estL) != nn) stop(paste0("Two edgeLists have different number of nodes.", 
                                       "True graph has ", nn, "; estimated graph has", length(estL), "."))
    if(!is.null(q)) {
        if(length(q) != nn) stop("Wrong permute order!")
        else {
            ## o is the new order of nodes
            ## i is at position q[i] in vector o
            ## so edge i->j in the new graph is q[i] -> q[j] in the original graph
